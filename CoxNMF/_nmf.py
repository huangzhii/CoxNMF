...
from ._newton_rhapson_for_efron_model import newton_rhapson_for_efron_model
import time
import warnings
import logging
...
def CoxNMF(X: np.ndarray,
    ...
    # print("W_init: %f, H_init: %f" % (np.sum(W_init), np.sum(H_init)))
    
    
    if W_init is None or H_init is None:
        print('Initialize W and H...')
        W, H = _initialize_nmf(X, n_components, init = 'random', random_state=random_state)
    else:
        W, H = W_init, H_init
    ...
    for n_iter in range(1, max_iter + 1):
        ...
        if (cindex - max_cindex) < - ci_tol: # if new concordance index smaller than previous certain value
        ...
        if cindex >= max_cindex:
            max_cindex = cindex
            max_cindex_res = {}
            max_cindex_res['W'] = copy.deepcopy(W)
            max_cindex_res['H'] = copy.deepcopy(H)
            max_cindex_res['error'] = error
            max_cindex_res['cindex'] = cindex
            max_cindex_res['relative_error'] = relative_error
            max_cindex_res['n_iter'] = n_iter
            max_cindex_res['beta'] = copy.deepcopy(beta).reshape(-1)
